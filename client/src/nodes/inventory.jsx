...
function Inventory({ id, data }) {
  ...
  const UNITS = {
    box: { label: "Box (100 pieces)", value: 100 },
    carton: { label: "Carton (12 Boxes)", value: 12 * 100 },
    roll: { label: "Roll (500 meters)", value: 500 },
    pallet: { label: "Pallet (50 Cartons)", value: 50 * 12 * 100 },
    drum: { label: "Drum (200 Litres)", value: 200 },
  };
...
// will impliment set reducer when learnt 
  const onUpdate = useCallback(
    (evt) => {
      ...
      setNodes((nodes) =>
        nodes.map((node) =>
          node.id === id
            ? { ...node, data: { ...node.data, value: finalValue, unit } }
            : node
        )
      );
    },
    [id, setNodes, unit]
  );//since reactflow stores the nodes datat inside the nodes array we need to updat the node array from the custom node to pass value onto other nodes when we connect them
  ...
  const onUnitChange = (evt) => {
    ...
    const multiplier = UNITS[newUnit].value;
    const finalValue = quantity * multiplier;

    setNodes((nodes) =>
      nodes.map((node) =>
        node.id === id
          ? { ...node, data: { ...node.data, value: finalValue, unit: newUnit } }
          : node
      )
    );
  };
